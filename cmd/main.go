...
func main() {
	...
	logger.Info(ctx, "应用启动 version=%s env=%s", "1.0.0", "development")
	logger.Debug(ctx, "调试信息 module=%s status=%s", "main", "initialized")
	logger.Warn(ctx, "警告信息 threshold=%d action=%s", 90, "monitor")
	logger.Error(ctx, "错误信息 error=%s retry=%v", "database connection failed", true)
	fmt.Println()

	// 带固定字段的日志
	fmt.Println("2. 带固定字段的日志")
	userLog := logger.With("user_id", "12345", "role", "admin")
	userLog.Info(ctx, "用户登录 ip=%s browser=%s", "192.168.1.1", "Chrome")
	userLog.Info(ctx, "用户操作 action=%s resource=%s", "create", "user")
	fmt.Println()
