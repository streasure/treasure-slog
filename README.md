...
基于 Go 原生 slog 实现的高性能日志库，具备与 zap 类似的功能特性，支持每秒百万级日志写入，核心路径仅 1 次内存分配。
...
- **高性能**：线程安全环形缓冲区 + 批量写入 + FastHandler 零反射序列化，核心路径仅 1 次内存分配
...
- **控制字段数量**：5个字段以内走零分配快速路径，核心路径仅 1 allocs/op
...
> 测试环境：Intel i5-10400F @ 2.90GHz, Windows, Go 1.x, 12 CPU cores

#### 核心性能指标

| 场景 | 吞吐量 | 延迟 | 内存分配 | 分配次数 |
|------|--------|------|----------|----------|
| 异步写入（2字段） | ~380万 ops/s | ~260 ns/op | 89 B/op | **1 allocs/op** |
| 全链路（5字段） | ~66万 ops/s | ~1500 ns/op | 182 B/op | 2 allocs/op |
| 不同级别 | ~480万 ops/s | ~210 ns/op | 48-56 B/op | 2 allocs/op |

#### 高并发性能

| 并发数 | 吞吐量 | 延迟 | 内存分配 | 分配次数 |
|--------|--------|------|----------|----------|
| 1 | ~135万 ops/s | ~740 ns/op | 113 B/op | 3 allocs/op |
| 2 | ~240万 ops/s | ~420 ns/op | 112 B/op | 3 allocs/op |
| 4 | ~360万 ops/s | ~280 ns/op | 112 B/op | 3 allocs/op |
| 8 | ~430万 ops/s | ~230 ns/op | 112 B/op | 2-3 allocs/op |
| 16 | ~430万 ops/s | ~230 ns/op | 112 B/op | 2 allocs/op |
| 32 | ~410万 ops/s | ~245 ns/op | 112 B/op | 2 allocs/op |
| 64 | ~390万 ops/s | ~255 ns/op | 112 B/op | 2 allocs/op |

#### 字段数量影响

| 字段数 | 延迟 | 内存分配 | 分配次数 |
|--------|------|----------|----------|
| 2 | ~275 ns/op | 112 B/op | 6 allocs/op |
| 4 | ~360 ns/op | 256 B/op | 11 allocs/op |
| 8 | ~580 ns/op | 544 B/op | 20 allocs/op |
| 16 | ~1075 ns/op | 1248 B/op | 38 allocs/op |
| 32 | ~2005 ns/op | 2848 B/op | 71 allocs/op |

> **建议**：字段数量控制在5个以内可获得最佳性能（核心路径仅1次分配）

#### 异步模式性能
- 平均响应时间：~260 ns/op
- 内存分配：89 B/op，1 allocs/op
- 吞吐量：380万+ 日志/秒

#### 全链路性能
- 平均响应时间：~1500 ns/op
- 内存分配：182 B/op，2 allocs/op

#### 不同日志级别性能
- Debug: ~195 ns/op，56 B/op，2 allocs/op
- Info: ~210 ns/op，48 B/op，2 allocs/op
- Warn: ~210 ns/op，48 B/op，2 allocs/op
- Error: ~215 ns/op，56 B/op，2 allocs/op

#### 高并发性能
- 8线程: ~230 ns/op，112 B/op，2-3 allocs/op
- 16线程: ~230 ns/op，112 B/op，2 allocs/op
- 32线程: ~245 ns/op，112 B/op，2 allocs/op
- 64线程: ~255 ns/op，112 B/op，2 allocs/op
